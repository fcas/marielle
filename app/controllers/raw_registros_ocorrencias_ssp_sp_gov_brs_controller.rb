class RawRegistrosOcorrenciasSspSpGovBrsController < ApplicationController
  ...
  def index
    @raw_registros_ocorrencias_ssp_sp_gov_brs = RawRegistrosOcorrenciasSspSpGovBr.where(nil)
    params.slice(:numero_boletim, :dataocorrencia, :bairro, :cidade, :uf, :rg)
    .each do |campo, valor|
      @raw_registros_ocorrencias_ssp_sp_gov_brs = @raw_registros_ocorrencias_ssp_sp_gov_brs.public_send("filter_by_#{campo}", valor) if valor.present?
    end

    respond_to do |format|
      format.html
      format.json { render json: @raw_registros_ocorrencias_ssp_sp_gov_brs, status: :ok }
      format.csv { send_data @raw_registros_ocorrencias_ssp_sp_gov_brs.as_csv, type: "text/csv", filename: "ocorrencias.csv" } 
    end
  end
